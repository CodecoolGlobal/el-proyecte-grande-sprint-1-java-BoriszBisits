...
function Profile() {
    ...
    const filterCoworkers = (query) => {
        const filtered = allCoworkers.filter((coworker) => coworker.toLowerCase().includes(query.toLowerCase()));
    ...
    const handleMessageSubmit = (e) => {
        ...
        const leader = localStorage.getItem("username");
        const dataToSend = {
            receiver: selectedCoworker.name, sender: leader, message: messageInput,
        };
        ...
        fetch(`/api/profile/sendmessage`, {
            method: "POST", headers: headers, body: JSON.stringify(dataToSend),
        });
    ...
    const handleInterestSubmit = (e) => {
        ...
        const leader = localStorage.getItem("username");
        const dataToSend = {
            interest: searchQuery, user: leader, subtype: subtypeInput,
        };
        const headers = {
            Authorization: `Bearer ${token}`, "Content-Type": "application/json",
        };
    ...
    const handleSubtypeSubmit = (e) => {
        ...
        const leader = localStorage.getItem("username");
        const dataToSend = {
            subtype: subtypeInput, user: leader,
        };
    ...
    const filterInterests = (query) => {
        const filtered = interest.filter((item) => item.toLowerCase().includes(query.toLowerCase()));
    ...
    const handleSaveUsername = async () => {
        ...
        try {
            const response = await fetch(`/api/profile/changeusername/${leader}`, {
                method: "PUT", headers: headers, body: JSON.stringify(dataToSend),
            });
            ...
            <div style={{display: "flex"}}>
                ...
                <div style={{flex: 1, padding: "20px"}}>
                    ...
                    <Container maxWidth="sm" style={{marginTop: "20px"}}>
                        <Typography variant="overline" display="block" style={{ color: 'blue', textAlign: 'center', fontSize: "1.4rem" }}>{profile.name + "'s Profile"}</Typography>
                        <img
                        ...
                        {/* Settings */}
                        <Typography variant="overline" display="block" style={{ color: 'blue', textAlign: 'center', marginTop: "10vh", fontSize: "1.3rem" }}>
                            Settings
                        </Typography>
                ...
                {/* Right Side */}
                <div style={{flex: 1, padding: "20px"}}>
                    {profile && profile.types && profile.types.length > 0 && (<div>
                            <Typography variant="h6">Your Interest Types:</Typography>
                            <ul>
                                {profile.types.map((projectType, index) => (<li key={index}>{projectType.name}</li>))}
                            </ul>
                        </div>)}

                    {profile && profile.subTypes && profile.subTypes.length > 0 && (<div>
                    ...
                    <form onSubmit={handleInterestSubmit}>
                        <Autocomplete
                            ...
                            value={filteredInterests.find((option) => option.name === searchQuery) || null}
                        ...
                        {savedInterest && (<Typography variant="body1">
                    ...
                    <form onSubmit={handleSubtypeSubmit}>
                        <Autocomplete
                            ...
                            options={subtypes}
                            getOptionLabel={(option) => option.name}
                            value={subtypes.find((option) => option.name === subtypeInput) || null}
                            onChange={(event, newValue) => setSubtypeInput(newValue ? newValue.name : "")}
                            renderInput={(params) => (<TextField
                                    {...params}
                        ...
                        {subtypeInput && (<Typography variant="body1">
